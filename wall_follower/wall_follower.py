...
class WallFollower(Node):
    ...
    def laser_callback(self,laserscan):
        ...
        if self.SIDE > 0: ##left
            x = right*np.cos(angles[3*length_wall//2:12*length_wall//5])
            y = right*np.sin(angles[3*length_wall//2:12*length_wall//5])
            # y = y[y<3*self.DESIRED_DISTANCE]
        else:
            x = left*np.cos(angles[length_wall//2:7*length_wall//5])
            y = left*np.sin(angles[length_wall//2:7*length_wall//5])
            # y = y[y<3*self.DESIRED_DISTANCE]
